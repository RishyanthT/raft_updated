...
class Node(threading.Thread):
                ...
                if self.is_active:
                    ...
                    if self.state == NodeState.LEADER:
                        if current_time - self.last_heartbeat >= 5:  # Heartbeat interval
                            self.send_heartbeat()
                            self.send_event("HEARTBEAT")  # Sending event via gRPC
    ...
    def send_event(self, event_type):
        """Send events to the gRPC server and log them in a file."""
        with grpc.insecure_channel('localhost:50051') as channel:
            stub = service_pb2_grpc.NodeCommunicationStub(channel)
            event = service_pb2.NodeEvent(id=self.id, type=event_type, term=self.term)
            response = stub.SendEvent(event)
            print(f"Sent event: {response.type} from node {response.id}, term: {response.term}")
            
            # Record heartbeat in a text file
            if event_type == "HEARTBEAT":
                with open('heartbeats_log.txt', 'a') as log_file:
                    log_file.write(f"Heartbeat: Node {self.id} sent heartbeat for term {self.term} at {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
    ...
    def send_vote_request(self):
        if self.state == NodeState.DEAD:
            return
        print(f"Node {self.id} sending vote request")
        self.last_vote_request = time.time()
    ...
    def start_election(self):
        ...
        print(f"Node {self.id} starting election with LOCK")
        self.send_vote_request()
